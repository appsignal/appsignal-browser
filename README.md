...
**Storage budget.** The in-memory rrweb buffer is naturally bounded by rrweb's checkout cadence (60 s of mutations between two `FullSnapshot`s). On every checkout the previous buffer is either shipped (sampled sessions) or dropped (unsampled `error_replay` sessions, since the old `FullSnapshot` it anchored on is gone). No size cap is needed — the buffer never grows beyond one checkout window. A separate 32 MB cap bounds the *transport* retry queue (failed-send chunks waiting for the network to recover); see *Retry* below for eviction semantics.
