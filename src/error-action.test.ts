...
describe("error action across one navigation", () => {
  ...
  // The trailing slash is dropped where the action is resolved, in vitals.ts,
  // so these drive the real route state rather than a mock of it.
  it("gives one group whether or not the pathname ends in a slash", () => {
    ...
    expect(errorActions()).toEqual(["/checkout"]);
  });
  ...
  it("removes the trailing slash from a route template too", () => {
    ...
    expect(errorActions()).toEqual(["/users/:id"]);
  });
});
